...
def dict2npy(cam_dict, gt_label, orig_img, th):
    ...
    # cam_npy = np.zeros((6, orig_img_size[0], orig_img_size[1]))
...
def dict2npy_no_bg(cam_dict, gt_label, orig_img, th):
    gt_cat = np.where(gt_label==1)[0]
    orig_img_size = cam_dict[gt_cat[0]].shape
...
def BCSS_dict2npy(cam_dict, gt_label, orig_img, th,n_class):
    gt_cat = np.where(gt_label==1)[0]
    ...
    # bg_score = [np.ones_like(cam_dict[gt_cat[0]])*th]
    bg_score = [gen_bg_mask(orig_img)]
    cam_npy = np.zeros((n_class, orig_img_size[0], orig_img_size[1]))
...
def cam_npy_to_label_map(cam_npy):
    seg_map = cam_npy.transpose(1,2,0)
    seg_map = np.asarray(np.argmax(seg_map, axis=2), dtype=np.uint8)
    return seg_map
...
def create_folder(inference_dir_path):
    ...
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map'))
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map/R0')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map/R0'))
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map/R1')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map/R1'))
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map/R2')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map/R2'))
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map/R3')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map/R3'))
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map/R4')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map/R4'))
    if not os.path.exists(os.path.join(inference_dir_path + '/attention_map/R5')):
        os.mkdir(os.path.join(inference_dir_path + '/attention_map/R5'))
...
def save_attention_map(cam, gt_label, save_path, img_name, round):
    gt_cat = np.where(gt_label==1)[0]
    for i, gt in enumerate(gt_cat):
        file_path = os.path.join(save_path, 'attention_map', 'R{}'.format(round), img_name + '_{}.npy'.format(gt))
        np.save(file_path, cam[gt])
    return
...
def cam2attention_map(x, T):
        
    x = torch.sigmoid(T*x) # 0 - 1
    x = (x - 0.5) / 0.5 # -1 - 1
    y = x + 1 # 0 - 2
