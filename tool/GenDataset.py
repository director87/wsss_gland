        ...
        for root, dirname, filename in os.walk(self.data_path):
            for f in filename:
                ...
                if self.dataset == 'luad':
                    ...
                    image_label = torch.Tensor([int(label_str[0]), int(label_str[1])])
                elif self.dataset == 'gcss':
                    image_label = torch.Tensor([int(label_str[0]), int(label_str[1]), int(label_str[2]), int(label_str[3]), int(label_str[4]), int(label_str[5])])
...
class Stage2_Dataset(Dataset):
    def __init__(self, args, base_dir, split):
        ...
        if self.split   == "train":
            ...
            # self._image_dir     = os.path.join(self._base_dir, 'train/')
            # self._image_dir     = os.path.join(self._base_dir, 'train_1p/')
            self._cat_dir       = os.path.join(self._base_dir, 'train_PM/PM_res38d_cambn7/')
            self._cat_dir_a     = os.path.join(self._base_dir, 'train_PM/PM_res38d_gpp_pam_camb5_2')
            self._cat_dir_b     = os.path.join(self._base_dir, 'train_PM/PM_res38d_gpp_pam_camb4_5')
            ...
            self._image_dir = os.path.join(self._base_dir, 'val_448/img/')
            self._cat_dir = os.path.join(self._base_dir, 'val_448/mask/')
        elif self.split == 'test':
            # self._image_dir = os.path.join(self._base_dir, 'test/img/')
            # self._cat_dir   = os.path.join(self._base_dir, 'test/mask/')
            self._image_dir = os.path.join(self._base_dir, 'test_448/img/')
            self._cat_dir = os.path.join(self._base_dir, 'test_448/mask/')
        self.args = args
