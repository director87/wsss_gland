...
def create_pseudo_mask(model, dataroot, fm, savepath, n_class, palette, dataset):
    ...
    for iter, (img_name, img_list) in enumerate(infer_data_loader):
        img_name = img_name[0]
        # img_path = os.path.join(os.path.join(dataroot,''),img_name.split('\\')[1] + '/' + img_name.split('\\')[2] + '.png')  # windows
        img_path = os.path.join(os.path.join(dataroot,'train_diffusion'),img_name + '.png')  # Linux
        # print(img_path)
        orig_img = np.asarray(Image.open(img_path))
        orig_img_size = orig_img.shape[:2]
